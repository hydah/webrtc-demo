...
public:
	virtual void onPusherEvent(RTCLiveEvent event) {
		...
		if (event == RTCLIVE_ENTERROOM) {
            Q_EMIT demoPusher->signalPublish();
		}
...
DemoPusher::DemoPusher(QMainWindow *parent)
    : QMainWindow(parent)
{
	bool ret = HasMemberToString<decltype(this)>::value;
    ui.setupUi(this);
	demoPusher = this;
	// add tool bar
	...
	livePusher->startPreview((WindowIdType)ui.m_localVideo->winId());
