...
bool AudioDeviceManager::init(rtc::Thread* worker_thread, PeerConnectionStream* peerconnect)
{
  ...
  m_audioDevice = m_workerThread->Invoke<rtc::scoped_refptr<webrtc::AudioDeviceModule>>(
          RTC_FROM_HERE, [&,peerconnect] {
            return webrtc::CreateAudioDeviceWithDataObserver(
                webrtc::AudioDeviceModule::AudioLayer::kPlatformDefaultAudio,webrtc::CreateDefaultTaskQueueFactory().get(),std::make_unique<MediaStreamImpl>(rtc::CreateRandomUuid(), peerconnect));
          });
...
VideoManagerImpl::VideoManagerImpl(PeerConnectionStream* livePusher)
{
